    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mb-3">
            ...
            <div class="input-group">
                ...
                <input id="name" class="form-control border-start-0 ps-0 @error('name') is-invalid @enderror" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" placeholder="{{ __('Nguyễn Văn A') }}">
            </div>
        ...
        <div class="row">
            ...
            <!-- Role -->
             <div class="col-md-6 mb-3">
                <label for="role" class="form-label fw-bold">{{ __('Bạn là?') }}</label>
                <select id="role" name="role" class="form-select @error('role') is-invalid @enderror" required>
                    <option value="">{{ __('-- Chọn --') }}</option>
                    <option value="patient" {{ old('role') == 'patient' ? 'selected' : '' }}>{{ __('Bệnh nhân') }}</option>
                    <option value="doctor" {{ old('role') == 'doctor' ? 'selected' : '' }}>{{ __('Bác sĩ') }}</option>
                </select>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="city_id" class="form-label fw-bold">{{ __('Thành phố') }}</label>
             ...
             <div class="col-md-6 mb-3">
                <label for="address" class="form-label fw-bold">{{ __('Địa chỉ') }}</label>
                <input id="address" class="form-control @error('address') is-invalid @enderror" type="text" name="address" value="{{ old('address') }}" required placeholder="{{ __('Số nhà, đường...') }}">
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                <label for="password_confirmation" class="form-label fw-bold">{{ __('Nhập lại') }}</label>
                <div class="input-group">
        ...
        <div class="d-grid gap-2 mt-4">
            <button type="submit" class="btn btn-primary btn-lg">
                {{ __('Đăng ký tài khoản') }} <i class="bi bi-person-plus-fill"></i>
            </button>
        </div>
        ...
        <div class="text-center mt-4">
            <p class="text-muted mb-0">
                {{ __('Đã có tài khoản?') }} 
                <a href="{{ route('login') }}" class="text-primary fw-bold text-decoration-none">{{ __('Đăng nhập ngay') }}</a>
            </p>
        </div>
    </form>
</x-guest-layout>
